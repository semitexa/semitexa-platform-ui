...
/**
 * Action-level sanitisation + allow-listing + response shape.
 *
 * Pins:
 *   - allow-list dropping (silent, no throw — defence in depth);
 *   - whitespace trimming;
 *   - empty-after-trim values dropped;
 *   - non-scalar values dropped;
 *   - generated id matches `uifs_<16hex>`;
 *   - debug carries submissionId + counts but NEVER raw values;
 *   - record stored exactly once per handle().
 */
final class PlatformDemoStoreContactActionTest extends TestCase
{
    private InMemoryUiFormDemoSubmissionRepository $repo;
    private PlatformDemoStoreContactAction $action;

    protected function setUp(): void
    {
        $this->repo = new InMemoryUiFormDemoSubmissionRepository();
        $this->action = new PlatformDemoStoreContactAction($this->repo);
    }

    private function context(array $values): UiFormSubmitActionContext
    {
        return new UiFormSubmitActionContext(
            formInstanceId: 'uci_demo_form_act',
            actionName:     PlatformDemoStoreContactAction::NAME,
            dispatchId:     'ui_evt_act_unit',
            values:         $values,
            fields:         [],
    ...
    {
        self::assertSame('platform.demo.storeContact', $this->action->name());
    }
    ...
    public function valid_submission_stores_one_record_and_returns_accepted(): void
    {
        ...
        self::assertSame(PlatformDemoStoreContactAction::MESSAGE, $result->message);
        ...
        self::assertSame(PlatformDemoStoreContactAction::NAME, $found->actionName);
        self::assertSame('uci_demo_form_act', $found->formInstanceId);
    }
    ...
    public function debug_payload_does_not_leak_class_or_repository_fqcn(): void
    {
        ...
        $serialised = json_encode($result->toDebug(), JSON_THROW_ON_ERROR);
        self::assertStringNotContainsString('PlatformDemoStoreContactAction', $serialised);
